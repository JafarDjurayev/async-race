...
export default function WinnersPage(): JSX.Element {
  const { winners, totalCount, loading, error, fetchWinners, currentPage, setCurrentPage } = useWinnerStore();
  ...
  return (
    <div className="winners-container">
      ...
      <table className="winners-table">
        ...
        <tbody>
          {winners.map((w, i) => ( <WinnerRow key={w.id} winner={w} index={i} page={currentPage}limit={limit} car={cars.find(c => c.id === w.id)} /> ))}
        </tbody>
      </table>
      {totalCount > limit && ( <Pagination currentPage={currentPage} totalPages={Math.ceil(totalCount / limit)} onPageChange={setCurrentPage} /> )}
    </div>
  );
}
